<div class="card shadow">
  <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
    ...
    <div class="float-right">
      ...
      <a href="<?php echo base_url('export/export_excel/'.encode_url('8b1'));?>" class="btn btn-success btn-icon-split btn-sm">
        <span class="icon text-white-50">
      ...
      <div class="modal-body">
        <div class="form-row">
          <div class="form-group col-md-12">
            <label for="mitra">Upload Excel File, Klik <a href="<?php echo base_url('assets/upload/8.b.1.prestasi_akad_mhs.xlsx');?>" data-toggle="tooltip" data-placement="top" title="Download Template">disini</a> untuk unduh file template</label>
...
$(document).ready(function(){
  ...
  function show_data(){
    $.ajax({
      type  : 'ajax',
      url   : '<?php echo site_url('luaran/prestasi_akademik_data_list')?>',
      async : false,
      dataType : 'json',
      success : function(data){
        ...
        for(i=0; i<data.length; i++){
          ...
          '<td style="text-align:right;">'+
              '<a href="javascript:void(0);" class="btn btn-info btn-circle btn-sm item_edit" data-toggle="tooltip" data-placement="top" title="Edit" data-seq_id="'+data[i].seq_id+'" data-nama_kegiatan="'+data[i].nama_kegiatan+'" data-waktu="'+data[i].waktu+'" data-tingkat="'+data[i].tingkat+'" data-prestasi="'+data[i].prestasi+'"><i class="fas fa-search"></i></a>'+' '+
  ...
  $('#upload').submit(function(e){
    e.preventDefault();
      $.ajax({
        url:'<?php echo site_url('upload/excel_upload/'.encode_url('8.b.1'))?>',
        type:"post",
  ...
  $('#btn_update').on('click',function(){
    ...
    var nama_kegiatan = $('#nama_kegiatan_edit').val();
    var waktu = $('#waktu_edit').val();
    var tingkat = $('#tingkat_edit').val();
    var prestasi = $('#prestasi_edit').val();

    $.ajax({
      type : "POST",
      url  : "<?php echo site_url('luaran/prestasi_akademik_edit')?>",
      dataType : "JSON",
      data : {seq_id:seq_id, nama_kegiatan:nama_kegiatan, waktu:waktu, tingkat:tingkat, prestasi:prestasi},
      success: function(data){
        $('[name="nama_kegiatan_edit"]').val("");
        $('[name="waktu_edit"]').val("");
        $('[name="tingkat_edit"]').val("");
        $('[name="prestasi_edit"]').val("");
  ...
  //delete record to database
   $('#btn_delete').on('click',function(){
      ...
      $.ajax({
          type : "POST",
          url  : "<?php echo site_url('luaran/prestasi_akademik_delete')?>",
          dataType : "JSON",
          data : {seq_id:seq_id},
          success: function(data){
