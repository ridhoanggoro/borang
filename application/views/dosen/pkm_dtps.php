<div class="card shadow">
  <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
    ...
    <div class="float-right">
      <a href="javascript:void(0);" onclick="return e_pkm_dtps('0', '<?php echo encode_url('penelitian_dosen'); ?>')" class="btn btn-primary btn-icon-split btn-sm">
  ...
  function show_data(){
    $.ajax({
      ...
      success : function(data){
        var html = '';
        var i;
        var tbl = '<?php echo encode_url('pkm_dosen'); ?>';
        for(i=0; i<data.length; i++){
          html += '<tr>'+
              ...
              '<td>'+
                  '<a href="javascript:void(0);" onclick="return e_pkm_dtps(\'' + data[i].seq_id + '\',\'' + tbl + '\');" class="btn btn-info btn-circle btn-sm item_edit" data-toggle="tooltip" data-placement="top" title="Edit"><i class="fas fa-search"></i></a>'+' '+
...
// end upload data
 ...
 //Save Data
    ...
    $.ajax({
      type : "POST",
      url  : "<?php echo site_url('dosen/pkm_dtps_add')?>",
      type: "post",
      data: new FormData(this),
      processData: false,
      contentType: false,
